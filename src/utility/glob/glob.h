...
/// Broken symlinks are included in the results (as in the shell)
    std::vector<boost::filesystem::path> glob(const std::string &pathname);
...
/// symbolic links to directories.
    std::vector<boost::filesystem::path> rglob(const std::string &pathname);
...
/// Initializer list overload for convenience
    std::vector<boost::filesystem::path> glob(const std::initializer_list<std::string> &pathnames);
