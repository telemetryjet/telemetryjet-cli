...
std::vector<record_data_frame_t> SqliteDatabase::getDataFrames(int system_id) {
    ...
    try {
        SQLite::Statement query(*db, fmt::format("select * from data_frames where system_id={}", system_id));
...
void SqliteDatabase::updateDataFrame(record_data_frame_t dataFrame) {
    ...
    try {
        SQLite::Statement updateStatement(*db, "update data_frames set system_id=? where id=?");
        updateStatement.bind(1, dataFrame.system_id);
        updateStatement.bind(2, dataFrame.id);
