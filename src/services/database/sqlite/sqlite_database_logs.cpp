...
std::vector<record_log_t> SqliteDatabase::getRecentLogs(int system_id, int limit, int beforeId) {
    ...
    try {
        ...
        if (beforeId != -1) {
            whereConditions += fmt::format(" and id < {}", beforeId);
        }
        SQLite::Statement
            query(*db,
                  fmt::format("select * from logs where {} order by id desc limit {}",
                              whereConditions,
                              limit));
        while (query.executeStep()) {
            logs.push_back({query.getColumn(0),
                            query.getColumn(1),
                            query.getColumn(2),
                            query.getColumn(3),
                            query.getColumn(4)});
...
std::vector<record_log_t> SqliteDatabase::getLogs(int system_id) {
    ...
    try {
        SQLite::Statement query(*db,
                                fmt::format("select * from logs where system_id={}", system_id));
        while (query.executeStep()) {
            logs.push_back({query.getColumn(0),
                            query.getColumn(1),
                            query.getColumn(2),
...
void SqliteDatabase::updateLog(record_log_t log) {
    ...
    try {
        SQLite::Statement updateStatement(
            *db, "update logs set system_id=?, timestamp=?, level=?, message=? where id=?");
