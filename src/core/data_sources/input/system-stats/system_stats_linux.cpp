...
void SystemStatsDataSource::computeMemoryAvailable() {
    ...
    if (sysinfo(&infoObj) == 0) {
        ...
        write(std::make_shared<DataPoint>(fmt::format("{}.ram.available", id), timestamp, freeRam));
        write(std::make_shared<DataPoint>(fmt::format("{}.ram.used", id), timestamp, usedRam));
        write(std::make_shared<DataPoint>(fmt::format("{}.ram.total", id), timestamp, totalRam));
    }
}
